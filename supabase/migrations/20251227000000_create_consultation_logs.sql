...
-- Wrapped in DO block to avoid error if policy exists
do $$
begin
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'consultation_logs' and policyname = 'Users can view their own project consultation logs') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'consultation_logs' and policyname = 'Users can insert their own project consultation logs') then
  ...
  if not exists (select 1 from pg_policies where schemaname = 'public' and tablename = 'consultation_logs' and policyname = 'Users can update their own project consultation logs') then
