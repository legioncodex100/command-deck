...
import { Activity, ShieldCheck, Clock, FileText, Rocket, Brain, Database, Layout, Layers, Cpu, GitBranch } from "lucide-react";
            ...
            <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-8">
                ...
                <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                    {[
                        ...
                        <div
                            key={s.title}
                            className="flex items-center gap-3 p-3 rounded-lg bg-[#0a0a0a] border border-zinc-800/50 hover:bg-zinc-900/50 hover:border-emerald-500/30 transition-all group cursor-default"
                            title={s.desc}
                        >
                            <div className={`p-1.5 rounded-md ${s.status === 'Active' ? 'bg-emerald-500/10 text-emerald-400' : 'bg-zinc-800/50 text-zinc-500'}`}>
                                <s.icon className="h-3.5 w-3.5" />
                            </div>

                            <div className="flex-1 min-w-0">
                                <div className="text-[10px] font-bold text-zinc-300 truncate group-hover:text-emerald-300">
                                    {s.title}
                                </div>
                            </div>

                            <div className={`h-1.5 w-1.5 rounded-full shrink-0 ${s.status === 'Active' ? 'bg-emerald-500 shadow-[0_0_6px_rgba(16,185,129,0.6)]' : 'bg-zinc-700'}`} />
                        </div>
                    ))}
                </section>
