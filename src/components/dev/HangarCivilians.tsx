...
import { Users, UserPlus, Shield, Mail, Search, Trash2, RefreshCw } from "lucide-react";
import { inviteCivilian } from "@/app/actions/admin";
...
export function HangarCivilians() {
    ...
    const handleInvite = async (e: React.FormEvent) => {
        ...
        if (res.error) {
            ...
            setEmail("");
            // Refresh list in case specific logic adds them immediately (unlikely until they accept, but good practice)
            loadCivilians();
        }
        setInviting(false);
    };
                            ...
                            civilians.map(profile => (
                                <div key={profile.id} className="grid grid-cols-12 px-4 py-3 border-b border-zinc-800/30 items-center hover:bg-zinc-900/40 transition-colors group">
                                    <div className="col-span-5 flex items-center gap-3">
                                        <div className="h-8 w-8 rounded-full bg-zinc-800 flex items-center justify-center text-zinc-400 font-bold text-xs">
                                    ...
                                    <div className="col-span-3 text-xs text-zinc-500 font-mono">
